...
typedef struct
{
    void*       port_handle;      /**< A handle to the open port (COM or UDP). */
    uint8_t     num_endpoints;    /**< The number of endpoints present in the
                                       connected device. */
    Endpoint_t* endpoints;        /**< An array containing information
                                       about each endpoint present in the
                                       connected device. */

    /** pointer to a function to receive data */
    size_t(*get_data)(void* port_handle, void* data, size_t num_requested_bytes);

    /** pointer to a function to send data */
    size_t (*send_data)(void* port_handle, const void* data, size_t num_bytes);

    /** pointer to a function to close connection */
    void (*close)(void* port_handle);
} Instance_t;
 ...
 *
 * \param[in] protocol  The protocol object to recover.
 */
static void recover_from_receive_error(const Instance_t* protocol);
 ...
 *
 * \param[in] protocol         The protocol instance that wants to send the
 *                             message.
 ...
 */
static void send_message(const Instance_t* protocol, uint8_t endpoint,
 ...
 *
 * \param[in]  protocol         The protocol instance that wants to read a
 *                              message.
 ...
 */
static int32_t get_message(const Instance_t* protocol,
...
static void recover_from_receive_error(const Instance_t* protocol)
{
    ...
    {
        received_bytes = protocol->get_data(protocol->port_handle,
                         ...
                         const uint8_t* payload, uint16_t payload_size)
{
    ...
    /* allocate memory */
    size_t message_size = (size_t)4 + (size_t)payload_size + (size_t)2;
    uint8_t* message = malloc(message_size *sizeof(uint8_t));
    if (message == NULL)
        return;

    /* copy header, payload, and tail into one buffer */
    memcpy(message, message_header, sizeof(message_header));
    memcpy(message+sizeof(message_header), payload, payload_size);
    memcpy(message + sizeof(message_header) + payload_size, message_tail, sizeof(message_tail));

    /* send message */
    protocol->send_data(protocol->port_handle, message, message_size);
                           ...
                           Message_Info_t* message_info)
{
    ...
    // message_info should have valid values even if we return from this
    // function with an error code.
    message_info->endpoint     = 0;
    message_info->payload      = NULL;
    message_info->payload_size = 0;

    /* read message header */
    /* ------------------- */
    num_received_bytes = protocol->get_data(protocol->port_handle,
                                            message_header,
    ...
    {
        num_received_bytes +=
            protocol->get_data(protocol->port_handle,
                               message_header + num_received_bytes,
                               sizeof(message_header) - num_received_bytes);
    }
    ...
    {
        recover_from_receive_error(protocol);
...
/**
 * @brief Return copy of string
 *
 * The function is aequivalent to strdup. As strdup is a POSIX extension and
 * and not part of C99 we write our own implementation here.
 *
 * @param [in]  s       string
 * @retval      scopy   copy of s if successful
...
{
    Port_Info_t port_info = { 0 };

    if (strlen(port_name) <= 6)
        return port_info;

    if (strncmp(port_name, "udp://", 6) == 0)
    {

        port_info.is_udp = 1;
    }
    else  if (strncmp(port_name, "tcp://", 6) == 0)
    {
        port_info.is_tcp = 1;
    }
    else
    ...
    char* p = NULL;
    char* server_port = strclone(port_name + 6);

    if (server_port)
    {
        p = strchr(server_port, ':');
        if (p)
        {
            *p = '\0';
            if (strlen(server_port) <= sizeof(port_info.ip4addr))
            {
                strcpy(port_info.ip4addr, server_port);
                port_info.port = atoi(p + 1);
            }
        }
...
{
    Instance_t protocol_instance = { 0 };
    Message_Info_t message_info;
    ...
    int32_t new_handle;
    {
        /* open the COM port */
        /* ----------------- */
        com_t* com_port_handle = com_open(port_name);
        if(!com_port_handle)
            return PROTOCOL_ERROR_COULD_NOT_OPEN_COM_PORT;

        protocol_instance.port_handle = com_port_handle;
        protocol_instance.get_data = (size_t(*)(void*, void*, size_t))com_get_data;
        protocol_instance.send_data = (size_t (*)(void*, const void*, size_t))com_send_data;
        protocol_instance.close = (void (*)(void*))com_close;
    }
     ...
     */
    {
        ...
        send_message(&protocol_instance, 0, uQueryMessage, sizeof(uQueryMessage));
    }

    /* read and parse reply message from connected device */
    /* -------------------------------------------------- */
    message_info.payload = NULL;
    receive_status = get_message(&protocol_instance, &message_info);
        ...
        (message_info.payload[0] != CNST_MSG_ENDPOINT_INFO))
    {
        ...
        protocol_instance.close(protocol_instance.port_handle);
    ...
    for (uint8_t i = 0; i < protocol_instance.num_endpoints; ++i)
    {
        uint32_t j;
...
void protocol_disconnect(int32_t protocol_handle)
{
    ...
    {
        const Instance_t* instance = &handles[protocol_handle];
        /* close COM port */
        instance->close(instance->port_handle);
        handles[protocol_handle].port_handle = NULL;
                                          ...
                                          Firmware_Information_t* information)
{
     ...
     */
    {
        ...
        send_message(protocol, 0, uQueryMessage, sizeof(uQueryMessage));
    }

    /* read and parse reply message from connected device */
    message_info.payload = NULL;
    receive_status = get_message(protocol, &message_info);
                                  ...
                                  uint16_t payload_size)
{
    ...
    /* send message */
    send_message(protocol, endpoint, payload, payload_size);

    /* receive messages from the board */
    while ((status_code = get_message(protocol, &message_info)) ==
                                                 ...
                                                 int32_t status_code)
{
    ...
    {
        // if status code is negative, an communication error has occurred
        // --------------------------------------------------------------
...
/*
 * TODO: provide the time-out as parameter, then the loop control get's much simpler!
 */

int32_t protocol_poll_receive_once(int32_t protocol_handle)
{
    Instance_t * protocol;
    Message_Info_t message_info;
    int32_t status_code;
    ...
    /* receive messages from the board */
    status_code = get_message(protocol, &message_info); //com_port_handle
    
    if (status_code == CNST_PROTOCOL_RECEIVED_PAYLOAD_MSG) 
    {
